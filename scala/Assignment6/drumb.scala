...
def get_january_data(symbol: String, year: Int) : List[String] = Try(Source.fromFile(symbol + ".csv").getLines.toList.filter(_.startsWith(year + "-"))).getOrElse(Nil)
...
def get_first_price(symbol: String, year: Int) : Option[Double] = {
    val lst = get_january_data(symbol, year)
    if (lst.length == 0) None else Some(lst.head.split(",").toList.tail.mkString("").toDouble)
}
...
def get_prices(portfolio: List[String], years: Range) : List[List[Option[Double]]] = {
    val yearList = for (i <- years.toList) yield {
        for (j <- portfolio) yield get_first_price(j, i)
    }
    yearList
}
