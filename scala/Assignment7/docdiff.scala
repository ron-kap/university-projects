...
def occurrences(xs: List[String]): Map[String, Int] = xs.distinct.map(i => (i, xs.count(_ == i))).toMap
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    ...
    if (map1.size >= map2.size) {
        //largest = map1
        val dotP = for (i <- map1) yield {
                        if (map2.get(i._1) == None) 0 else (i._2) * (map2.get(i._1).get)
                    }
        dotP.sum
    } else {
        //largest = map2
        val dotP = for (i <- map2) yield {
                        if (map1.get(i._1) == None) 0 else (i._2) * (map1.get(i._1).get)
                    }
        dotP.sum
    }
}
...
def overlap(lst1: List[String], lst2: List[String]) : Double = if (prod(lst1, lst1) >= prod(lst2, lst2)) prod(lst1, lst2).toDouble / prod(lst1, lst1) else prod(lst1, lst2).toDouble / prod(lst2, lst2)
