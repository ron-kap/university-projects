...
def get_csv_url(url: String) : List[String] = Try(Source.fromURL(url)("ISO-8859-1").getLines.toList.tail).getOrElse(Nil)
...
def process_ratings(lines: List[String]) : List[(String, String)] = {
    val ret = for (i <- lines) yield {
        val value = i.split(",")
        (value(0).toString, value(1).toString, value(2).toString)
    }
    
    for (i <- (ret.filter(_._3 == "4") ::: ret.filter(_._3 == "5"))) yield {
        i match {
            case (x, y, _) => (x, y)
        }
    }
